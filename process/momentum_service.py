...
class MomentumService:
    ...
    def hent_krav_til_jobsøgning(self, borger) -> int | None:
        ...
        if krav_tekst is None or len(krav_tekst) == 0:
            self.opret_opgave_til_sagsbehandler(
                borger, "'Krav til jobsøgning' blev ikke fundet."
            )
        ...
        if krav_antal is None:
            self.opret_opgave_til_sagsbehandler(
                borger,
                "Der mangler oplysninger om antallet af jobs i 'Krav til jobsøgning'.",
            )
    ...
    def hent_joblog_aktiviteter(self, borger) -> int:
        ...
        now = datetime.now(timezone.utc).astimezone(copenhagen_tz)

        # Previous month boundaries in Copenhagen time
        start_dato = (now.replace(day=1) - timedelta(days=1)).replace(
            day=1, hour=0, minute=0, second=0, microsecond=0
        )
        slut_dato = now.replace(
            day=1, hour=0, minute=0, second=0, microsecond=0
        ) - timedelta(microseconds=1)

        tidligere_måneds_joblog = []

        for entry in joblog:
            # createdAt → Copenhagen time
            created_at = self.__parse_date(entry.get("createdAt"))
            if created_at.tzinfo is None:
                created_at = created_at.replace(tzinfo=timezone.utc)
            created_at = created_at.astimezone(copenhagen_tz)

            # submissionDate → UTC → Copenhagen time
            submission_raw = entry.get("submissionDate")
            if not submission_raw:
                continue

            submission_utc = datetime.fromisoformat(
                submission_raw.replace("Z", "+00:00")
            )
        ...
        for joblog_entry in tidligere_måneds_joblog:
            job_hash = (
                f"{joblog_entry.get('title', '')} "
                f"{joblog_entry.get('companyName', '')} "
        ...
        return len(unikke_jobs)

    def kontroller_jobsøgning(
        self, borger: dict, krav_til_jobsøgning: int, antal_søgte_jobs: int
    ):
        if krav_til_jobsøgning > 0 and antal_søgte_jobs == 0:
            self.opret_opgave_til_sagsbehandler(
                borger, "Der var ikke registreret nogen jobs i joblog."
            )
            ...
            self.opret_opgave_til_sagsbehandler(
                borger, "Der er registreret for få job i joblog."
            )
